...
def handleEchoServer(echosocket, relayhost, relayport):
    ...
    tunnelport = relayport + 1; #bms: tunnel client와 접속할 포트번호
    ...
    echosocket.send(tunnel_msg) #bms: echoserver에 메시저 전송
        
    # Accept connection
    tunnelclient, tunnelclientaddr = tunnelsocket.accept()
    print("Client IP:PORT is...")
    print(tunnelclientaddr)
...
def createRelayServer(relayhost, relayport):
    ...
    while running:
        # Accept connection
        echosocket, echoaddr = relaysocket.accept() #bms: echoserver에서 접속하는 소켓
